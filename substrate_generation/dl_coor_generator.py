...
class DictionaryAnalyzer:
    """
    Applies Dictionary Learning to environment data to determine
    substrate coordinates.
    """
    def __init__(self, data, obs_size, act_size, feature_dims, alpha=1.0, max_iter=1000, width_factor=1.0, normalize_coors=True, depth_factor=1):
    ...
    def generate_io_coordinates(self):
        """
        Runs Dictionary Learning to generate feature coordinates (loadings on each atom)
        and then augments them with a dedicated dimension for network layering.
        """
        print(
            f"Running Dictionary Learning to find {self.feature_dims} dictionary atoms..."
        )
        
        # Standardize and fit the DL model
        scaler = StandardScaler()
        scaled_data = scaler.fit_transform(self.data)
        
        # transform_alpha is the L1 regularization term that promotes sparsity.
        self.dl = DictionaryLearning(
        ...
        # For consistency with PCA/FA to get node coordinates the model is fitted on the samples.
        self.dl.fit(scaled_data)

        print("DL complete. Extracting coordinates (loadings on dictionary atoms).")

        # Extract feature coordinates. The dictionary atoms are in .components_
        # Shape is (n_components, n_features). Transpose to get coordinates for each node.
        all_feature_coors = self.dl.components_.T
    ...
    def plot_dictionary_atoms(self, save_path: str):
        """
        Generates a heatmap of the learned dictionary atoms.
        """
        if self.dl is None:
